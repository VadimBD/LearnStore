...
namespace LearnStore.Infrastructure.DataAccess.MsSql
{
    public class AppDbContextFactory : IDesignTimeDbContextFactory<AppDbContext>
    {
        ...
        {
            var connectionString = configuration.GetConnectionString("LernStoreMigration")
                                    ?? throw new InvalidOperationException("Conection string 'LernStoreMigration' not found.");
            ...
            if (!string.IsNullOrEmpty(migrationUserName) && migrationUserName == builder.UserID && string.IsNullOrEmpty(builder.Password) && !builder.IntegratedSecurity) 
            {
                ...
                builder.Password = _passwordProvider.GetPassword("migration_password");
            }
